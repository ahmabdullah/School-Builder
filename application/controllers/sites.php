...
class Sites extends CI_Controller {
	...
	*/
	public function assign() {
            ...
            $user_id = $this->input->post('assigned_user_id');
            $site_id = $this->input->post('siteID');
            $site_name = $this->input->post('siteName');
            $user_name = $this->input->post('assigned_user');
            
            //Assign existing site to user
            $newSiteID = $this->sitemodel->assignToUser($user_id, $site_id);
            
            //all good then
            $return = array();

            $temp = array();
            $temp['header'] = $this->lang->line('site_assign_success_heading');
            $temp['content'] = $site_name . $this->lang->line('site_assign_success_message') . $user_name;
	...
	*/
	...
	public function tsave() {
		...
		if( isset($_POST['pagesData']) ) {
			...
			$templateID = $this->pagemodel->saveTemplate( $_POST['siteName'], $_POST['pageData'], $_POST['contents'], $_POST['pagesData'], $_POST['templateID'] );
			
		} else {
			
			$templateID = $this->pagemodel->saveTemplate( $_POST['siteName'], $_POST['pageData'], $_POST['contents'], '', $_POST['templateID'] );
	...
	public function save( $forPublish = 0 ) {
		...
		if( $_POST['siteID'] == 0 ) {//no siteID provided, creste a new site
			...
			if( isset($_POST['pagesData']) ) {
				...
				$this->sitemodel->update($siteID, $_POST['siteName'], $_POST['pageData'], $_POST['contents'], $_POST['pagesData']);
			
			} else {
							
				$this->sitemodel->update($siteID, $_POST['siteName'], $_POST['pageData'], $_POST['contents']);
	...
	*/
	...
	public function preview($siteID, $page) {
	...
	public function previewu($siteID, $page) {
		
		if( !$this->sitemodel->isMine( $siteID ) && !$this->ion_auth->is_admin() ) {
			
			die('You can not do this');
			
		}
				
		$page = $this->sitemodel->getPage($siteID, $page);
		
		if( $page ) {
		
			echo $page->pages_preview;
			
		} else {
			
			echo "This page does not have a preview. Please save the page first.";
