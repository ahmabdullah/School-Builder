...
<body>
    ...
    <div id="builder">
        ...
        <!-- Left Sidebar only for admin -->
        <?php if ($this->ion_auth->is_admin() || $this->config->config['sidebar_for_users']) { ?>
            ...
            <header class="clearfix" data-spy="affix" data-offset-top="60" data-offset-bottom="200">
                ...
                <div class="btn-group" style="float: right;">           
                    ...
                    <ul class="dropdown-menu dropdown-inverse">
                        <li><a href="#siteSettings" id="siteSettingsButton" class="siteSettingsModalButton" data-siteid="<?php echo $siteData['site']->sites_id; ?>"><span class="fui-arrow-right"></span> <?php echo $this->lang->line('actionbuttons_sitesettings') ?></a></li>
                        <li><a href="#pageSettingsModal" id="pageSettingsButton" data-toggle="modal" data-siteid="<?php echo $siteData['site']->sites_id; ?>"><span class="fui-arrow-right"></span> <?php echo $this->lang->line('actionbuttons_pagesettings') ?></a></li>
                    </ul>
                </div>

                <a href="#previewModal" data-toggle="modal" class="btn btn-inverse btn-embossed pull-right" style="margin-right: 10px;"><span class="fui-window"></span> <?php echo $this->lang->line('actionbuttons_preview') ?></a>

                <?php if ($this->ion_auth->is_admin()): ?>
                    <div class="btn-group" style="float: right; margin-right: 10px;">           
                        <button class="btn btn-primary btn-embossed"><span class="fui-check"></span> <span class="bLabel"><?php echo $this->lang->line('actionbuttons_save') ?></span></button>
                ...
                <a href="#exportModal" id="exportPage" data-toggle="modal" class="btn btn-info btn-embossed pull-right disabled actionButtons" style="margin-right: 1px"><span class="fui-export"></span> <?php echo $this->lang->line('actionbuttons_export') ?></a>

                <a href="#" id="publishPage" class="btn btn-info btn-embossed pull-right disabled actionButtons" data-siteid="<?php echo $siteData['site']->sites_id; ?>" <?php if ($siteData['site']->ftp_ok == 0): ?>data-toggle="tooltip"<?php endif; ?> data-placement="bottom" title="You can not publish your site right now. Please update your FTP details." style="margin-right: 1px"><span class="fui-upload"></span> <?php echo $this->lang->line('actionbuttons_publish') ?> <span class="fui-alert text-danger" <?php if ($siteData['site']->ftp_ok == 1): ?>style="display:none"<?php endif; ?>></span></a>
                            ...
                            <?php foreach ($siteData['pages'] as $page => $frames): ?>
                                <ul id="page<?php echo $counter; ?>" class="ui-sortable" style="display: <?php if ($counter == 1): ?>block<?php else: ?>none<?php endif; ?>;">
                                    <?php foreach ($frames as $frame): ?>
                                        <li class="element">
                                            <iframe id="<?php echo "ui-id-" . ($c + 10000); ?>" frameborder="0" <?php if ($frame->frames_sandbox == 1): ?>data-loaderfunction="<?php echo $frame->frames_loaderfunction; ?>" data-sandbox="<?php
                                                $frame->sandboxID = rand(0, 999999999);
                                                echo $frame->sandboxID;
                                                ?>"<?php endif; ?> scrolling="0" src="<?php echo site_url('sites/getframe/' . $frame->frames_id) ?>" data-height="<?php echo $frame->frames_height; ?>" data-originalurl="<?php echo $frame->frames_original_url ?>"></iframe>
                                        </li>
                                        <?php $c++; ?>
                                    <?php endforeach; ?>
                                </ul>
...
<button type="button" class="btn btn-default btn-embossed btn-block btn-sm" id="closeStyling"><span class="fui-cross-inverted"></span> Close Editor</button>-->
    ...
    <div class="modal fade" id="previewModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default btn-embossed" data-dismiss="modal"><span class="fui-cross"></span> <?php echo $this->lang->line('modal_cancelclose') ?></button>
                    <a href="" data-defurl="<?php echo site_url('/sites/preview/' . $siteData['site']->sites_id) ?>/" target="_blank" class="btn btn-primary btn-embossed" id="pagePreviewLink"><span class="fui-export"></span> <?php echo $this->lang->line('modal_preview_button') ?></a>
                </div>
    ...
    <div class="sandboxes" id="sandboxes" style="display: none">

        <?php foreach ($siteData['pages'] as $page => $frames): ?>
            <?php foreach ($frames as $frame): ?>
                <?php if ($frame->frames_sandbox == 1): ?>
                    <iframe id="<?php echo $frame->sandboxID ?>" sandbox="allow-same-origin" src="<?php echo site_url('sites/getframe/' . $frame->frames_id) ?>"></iframe>
                <?php endif; ?>
            <?php endforeach; ?>
        <?php endforeach; ?>
    </div>
    ...
    <script src="<?php echo base_url(); ?>js/redactor/table.js"></script>
    <script src="<?php echo base_url(); ?>js/redactor/bufferButtons.js"></script>
    <script src="<?php echo base_url(); ?>js/src-min-noconflict/ace.js"></script>
    <script src="<?php echo site_url('getelements'); ?>"></script>
    <script src="<?php echo base_url(); ?>js/builder.js"></script>
    <script>
...
<?php if ($this->ion_auth->is_admin()): ?>
                ...
                $('#templates li').each(function () {
                    ...
                    newLink = $('<a href="#delTemplateModal" data-toggle="modal" data-pageid="' + $(this).attr('data-pageid') + '" class="btn btn-danger btn-sm">DEL</a>');

                    $(this).find('.zoomer-cover').append(newLink)

                })

                $('#delTemplateModal').on('show.bs.modal', function (event) {
